...
El comportamiento del WAL se controla desde \texttt{postgresql.conf}.
Los parámetros más críticos afectan cuánto WAL se retiene, con qué frecuencia se hacen checkpoints, y si se comprime.
Un checkpoint frecuente (cada minuto) reduce el tiempo de recovery ante un crash pero aumenta la escritura en disco.
Un checkpoint poco frecuente (cada 30 minutos) reduce el I/O pero alarga el tiempo de recovery y puede generar picos de I/O cuando se dispara.
El valor por defecto de 5 minutos es un balance razonable para la mayoría de sistemas.
...
Un base backup por sí solo solo permite restaurar al momento exacto en que se tomó.
Para recuperarse a cualquier punto intermedio (``restore to 14:23 yesterday''), necesitas además los segmentos WAL generados desde el backup hasta ese momento.
El archivado de WAL automatiza esto: cada vez que PostgreSQL completa un segmento de 16MB, ejecuta el \texttt{archive\_command} para copiarlo al destino externo (S3, NFS, etc.).
...
pgBackRest resuelve en un solo tool los problemas que de otra forma requieren scripts caseros frágiles: coordinación de base backup + archivado WAL, compresión eficiente, cifrado at-rest, backups diferenciales e incrementales para reducir tiempo y almacenamiento, y verificación de integridad del repositorio.
Es la herramienta estándar de facto en producción para PostgreSQL, superando a la alternativa más antigua \texttt{barman}.
...
PITR (Point-In-Time Recovery) es la capacidad de restaurar PostgreSQL al estado exacto que tenía en un momento específico del pasado.
Es la defensa principal contra errores humanos: un \texttt{DELETE} sin WHERE, un \texttt{DROP TABLE} accidental, o datos corruptos por un bug de la aplicación.

El proceso combina un base backup (que establece el estado en un punto conocido) con la aplicación secuencial de segmentos WAL (que reproducen cada cambio posterior) hasta el instante deseado.
PostgreSQL aplica el WAL en modo recovery y cuando alcanza el \texttt{recovery\_target\_time}, se detiene y promueve la instancia como primary.
Toda la data entre el momento del backup y el recovery target queda disponible; la data escrita \emph{después} del target se descarta.
